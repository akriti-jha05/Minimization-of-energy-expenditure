...
function cost = fitnessFunction(position, positions, alpha, beta)
    ...
    cost = alpha *( maxDist/707.1)+ beta * maxHop;
end
...
% Varying number of gateway from 60 to 140
Gateway_counts = 60:20:140;
round_result = zeros(length(Gateway_counts), size(base_station_locations, 25));
round_results_gwo = zeros(length(Gateway_counts), size(base_station_locations, 1));
...
for i = 1:size(base_station_locations, 1)
    ...
    for j = 1:length(Gateway_counts)
       ...
       numParticles = numGateways;    % Number of particles in the swarm
        ...
        [Xbest, bestCost] = gwoRouting(numSensors, numGateways, maxIter, alpha, beta, base_station, network_range);
        ...
        round_result(j,i , k) = rounds;
         ...
         round_results_gwo(j,i) = mean(round_result(j,i,:));
