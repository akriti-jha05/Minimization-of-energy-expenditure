...
% Varying number of gateways from 60 to 140
Gateway_counts = 60:20:140;
round_result = zeros(length(Gateway_counts), size(base_station_locations, 25));
round_results_cray_optimizer = zeros(length(Gateway_counts), size(base_station_locations, 1));
...
for i = 1:size(base_station_locations, 1)
    ...
    for j = 1:length(Gateway_counts)
       ...
       for k = 1:7
        ...
        % Store the result
        unused_gateways_results(j,i ,k) = unused_gateways;
       end
       round_results_cray_optimizer(j,i) =  mean(round_result(j,i,:));
    end
end
