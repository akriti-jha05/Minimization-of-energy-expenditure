...
% Varying number of gateway from 10 to 700
Gateway_counts = 60:20:140;
round_result = zeros(length(Gateway_counts), size(base_station_locations, 25));
round_results_dist = zeros(length(Gateway_counts), size(base_station_locations, 1));
...
for i = 1:size(base_station_locations, 1)
    ...
    for j = 1:length(Gateway_counts)
        ...
        [round_result(j,i , k), ~] = Getroundnum(numSensors, numGateways, base_station);
        end
         round_results_dist(j,i) = mean(round_result(j,i,:));
    end
end
